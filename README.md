...
│   ├── TurretSubsystem.java        — Rotates the shooter to face the hub. Uses Motion Magic position control.
...
- The turret uses Motion Magic closed-loop position control (smooth trapezoidal/S-curve profiles) with two PID slots:
  - Slot 0 (normal aim): KP=30, KI=0.5, KD=0.2, KV=1.3, KA=0.1 — aggressive tracking with velocity feedforward
  - Slot 1 (reset moves): KP=8, KD=0.5, KV=0.5 — gentle approach to avoid overshoot
  - Motion Magic limits: cruise 0.4 rps (144°/sec), accel 1.0 rps/s, jerk 10 rps/s/s — prevents gear teeth from skipping on the small turret pinion
...
- **Trajectory Physics (`passAutoAim`)** — the experimental method. Uses real projectile math in `TrajectoryCalculations.java` to solve for the required exit velocity and launch angle given the distance and height difference. Better for shots at longer/unusual distances where the interpolation tables weren't calibrated. If the physics solver can't find a solution, it automatically falls back to `autoAim`.

The co-pilot Back button toggles between these two methods at runtime (defaults to OFF = interpolation tables). The toggle applies to ALL shots — hub and passing alike. The current mode is shown on the dashboard as "Trajectory Passing". During autonomous, interpolation tables are always used regardless of the toggle.

**To make trajectory physics accurate, you need to:**
1. Count the pulley teeth on the flywheel system — done: 24T motor, 15T wheel (1.6:1 ratio, HTD 5mm belt)
2. Measure the actual hood angle range — done: servo 0.0 = ~77.5° from horizontal, servo 1.0 = ~67.5° (10° total range, inverted)
3. Measure `TURRET_HEIGHT_METERS` from the floor to the ball exit point — done: 21 inches (0.5334m)
